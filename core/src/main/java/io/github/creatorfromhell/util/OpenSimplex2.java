...
public class OpenSimplex2 {
  ...
  public static float noise2_ImproveX(final long seed, final double x, final double y) {
    ...
    final double xx = x * ROOT2OVER2;
    final double yy = y * (ROOT2OVER2 * (1 + 2 * SKEW_2D));
  ...
  private static float noise2_UnskewedBase(final long seed, final double xs, final double ys) {
    ...
    final long xsbp = xsb * PRIME_X;
    final long ysbp = ysb * PRIME_Y;

    // Unskew.
    final float t = (xi + yi) * (float)UNSKEW_2D;
    final float dx0 = xi + t;
    final float dy0 = yi + t;

    // First vertex.
    float value = 0;
    final float a0 = RSQUARED_2D - dx0 * dx0 - dy0 * dy0;
    if(a0 > 0) {
      value = (a0 * a0) * (a0 * a0) * grad(seed, xsbp, ysbp, dx0, dy0);
    }

    // Second vertex.
    final float a1 = (float)(2 * (1 + 2 * UNSKEW_2D) * (1 / UNSKEW_2D + 2)) * t + ((float)(-2 * (1 + 2 * UNSKEW_2D) * (1 + 2 * UNSKEW_2D)) + a0);
    if(a1 > 0) {
      final float dx1 = dx0 - (float)(1 + 2 * UNSKEW_2D);
      final float dy1 = dy0 - (float)(1 + 2 * UNSKEW_2D);
      value += (a1 * a1) * (a1 * a1) * grad(seed, xsbp + PRIME_X, ysbp + PRIME_Y, dx1, dy1);
    }

    // Third vertex.
    if(dy0 > dx0) {
      final float dx2 = dx0 - (float)UNSKEW_2D;
      final float dy2 = dy0 - (float)(UNSKEW_2D + 1);
      final float a2 = RSQUARED_2D - dx2 * dx2 - dy2 * dy2;
      if(a2 > 0) {
        value += (a2 * a2) * (a2 * a2) * grad(seed, xsbp, ysbp + PRIME_Y, dx2, dy2);
      }
    } else {
      final float dx2 = dx0 - (float)(UNSKEW_2D + 1);
      final float dy2 = dy0 - (float)UNSKEW_2D;
      final float a2 = RSQUARED_2D - dx2 * dx2 - dy2 * dy2;
      if(a2 > 0) {
  ...
  public static float noise3_ImproveXY(final long seed, final double x, final double y, final double z) {
    ...
    final double xy = x + y;
    final double s2 = xy * ROTATE_3D_ORTHOGONALIZER;
    final double zz = z * ROOT3OVER3;
    final double xr = x + s2 + zz;
    final double yr = y + s2 + zz;
    final double zr = xy * -ROOT3OVER3 + zz;
  ...
  public static float noise3_ImproveXZ(final long seed, final double x, final double y, final double z) {
    ...
    // Orthonormal rotation. Not a skew transform.
    final double xz = x + z;
    final double s2 = xz * ROTATE_3D_ORTHOGONALIZER;
    final double yy = y * ROOT3OVER3;
    final double xr = x + s2 + yy;
    final double zr = z + s2 + yy;
    final double yr = xz * -ROOT3OVER3 + yy;
  ...
  public static float noise3_Fallback(final long seed, final double x, final double y, final double z) {
    ...
    // Orthonormal rotation. Not a skew transform.
    final double r = FALLBACK_ROTATE_3D * (x + y + z);
    final double xr = r - x;
    final double yr = r - y;
    final double zr = r - z;
   ...
   */
  private static float noise3_UnrotatedBase(long seed, final double xr, final double yr, final double zr) {

    // Get base points and offsets.
    final int xrb = fastRound(xr);
    final int yrb = fastRound(yr);
    final int zrb = fastRound(zr);
  ...
  public static float noise4_ImproveXYZ_ImproveXY(final long seed, final double x, final double y, final double z, final double w) {
    ...
    final double xy = x + y;
    final double s2 = xy * -0.21132486540518699998;
    final double zz = z * 0.28867513459481294226;
    final double ww = w * 0.2236067977499788;
    final double xr = x + (zz + ww + s2);
    final double yr = y + (zz + ww + s2);
    final double zr = xy * -0.57735026918962599998 + (zz + ww);
  ...
  public static float noise4_ImproveXYZ_ImproveXZ(final long seed, final double x, final double y, final double z, final double w) {
    ...
    final double xz = x + z;
    final double s2 = xz * -0.21132486540518699998;
    final double yy = y * 0.28867513459481294226;
    final double ww = w * 0.2236067977499788;
    final double xr = x + (yy + ww + s2);
    final double zr = z + (yy + ww + s2);
    final double yr = xz * -0.57735026918962599998 + (yy + ww);
